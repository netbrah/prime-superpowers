...
function resolveReportAdmission(records, { admissionId, childId }) {
  const bindings = records.filter((record) => record.event === "child-bound");
  const childBinding = bindings.find((record) => record.detail?.childId === childId);
  const admissionBinding = admissionId
    ? bindings.find((record) => record.detail?.admissionId === admissionId)
    : null;
  if (childBinding && admissionId && childBinding.detail.admissionId !== admissionId) {
    fail("E_CONTROLLER_REQUIRED", "child is already bound to another admission");
  }
  if (admissionBinding && admissionBinding.detail.childId !== childId) {
    fail("E_CONTROLLER_REQUIRED", "admission is already bound to another child");
  }
  const selectedId = admissionId ?? childBinding?.detail.admissionId;
  const terminal = new Set(["completed", "failed", "cleanup-failed", "quarantined-late-report"]);
  const latestByTask = new Map();
  for (const record of records) {
    if (record.detail?.lifecycle) latestByTask.set(record.taskId, record.detail.lifecycle);
  }
  const live = [...latestByTask.entries()]
    .filter(([, lifecycle]) => !terminal.has(lifecycle.status))
    .map(([taskId, lifecycle]) => ({ taskId, lifecycle, admissionId: lifecycle.attemptId }));
  if (!selectedId && live.length > 1) {
    fail("E_ADMISSION_AMBIGUOUS", "multiple live admissions require --admission");
  }
  const selected = selectedId
    ? live.find((candidate) => candidate.admissionId === selectedId)
    : live[0];
  if (!selected) fail("E_CONTROLLER_REQUIRED", "report does not match a live admission");
  return {
    ...selected,
    alreadyBound: Boolean(childBinding || admissionBinding),
  };
}
...
export function createWorkflowController(options) {
  ...
  async function report({ admissionId, childId, status }) {
    required(childId, "childId");
    if (admissionId !== undefined) required(admissionId, "admissionId");
    if (!["ok", "fail"].includes(status)) fail("E_CONTROLLER_REQUIRED", "status must be ok or fail");
    const records = await readLedger(ledgerPath);
    let { taskId, lifecycle, admissionId: selectedAdmission, alreadyBound } =
      resolveReportAdmission(records, { admissionId, childId });
    if (!alreadyBound) {
      await appendLedger(ledgerPath, {
        runId,
        taskId,
        event: "child-bound",
        detail: { admissionId: selectedAdmission, childId },
      }, { clock });
    }
    if (status === "fail") {
      lifecycle = applyLifecycleEvent(lifecycle, "fail", { reason: `child ${childId} failed` }, { clock });
      await persist(taskId, "failed", lifecycle, { admissionId: selectedAdmission, childId });
    } else {
      if (["admitted", "queued", "retrying"].includes(lifecycle.status)) {
        lifecycle = applyLifecycleEvent(lifecycle, "running", {}, { clock });
        await persist(taskId, "running", lifecycle, { admissionId: selectedAdmission, childId });
      }
      let bytes;
      try {
      ...
      await persist(taskId, "reported", lifecycle, { admissionId: selectedAdmission, childId });
      lifecycle = applyLifecycleEvent(lifecycle, "complete", {}, { clock });
      await persist(taskId, "completed", lifecycle, { admissionId: selectedAdmission, childId });
    }
    await appendLedger(ledgerPath, {
      runId,
      taskId,
      event: "report-ack",
      detail: { admissionId: selectedAdmission, childId, status },
    }, { clock });
...
export function parseControllerArgs(argv) {
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    if (!["--task", "--model", "--admission", "--child", "--status"].includes(flag) ||
  ...
  if (command === "report" && [3, 4].includes(values.size) && values.has("--child") &&
      values.has("--status") && ["ok", "fail"].includes(values.get("--status")) &&
      (values.size === 3 || values.has("--admission"))) {
    return {
      command,
      ...(values.has("--admission") ? { admissionId: values.get("--admission") } : {}),
      childId: values.get("--child"),
