...
export async function discoverParentSession(socketPath, cwd) {
  const response = await daemonRequest({
    socketPath,
    command: { type: "list", all: false, cwd, includeClientOwned: true },
  });
  const matches = (response.data?.sessions ?? []).filter((session) =>
    session?.cwd === cwd && session.runtimeKind !== "subagent" && session.activeSessionId
  );
  if (matches.length !== 1) {
    fail("E_PARENT_SESSION_UNRESOLVED", "parent session identity is missing or ambiguous");
  }
...
async function waitForParentSession(socketPath, cwd, { attempts = 100, delayMs = 100 } = {}) {
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    try {
      ...
      lastError = error;
      if (!["E_DAEMON_UNREACHABLE", "E_PARENT_SESSION_UNRESOLVED"].includes(error?.code)) throw error;
      await new Promise((resolveDelay) => setTimeout(resolveDelay, delayMs));
    }
  }
  throw lastError ?? new LauncherError("E_PARENT_SESSION_UNRESOLVED", "parent session was not observed");
}
    ...
    socket.on("data", async (chunk) => {
      ...
      if (request?.type !== "depth_verdict" || Object.keys(request).length !== 1) {
        ...
        try {
          ...
          verdict = {
            ok: false,
            code: error?.code === "E_PARENT_SESSION_UNRESOLVED"
              ? "E_PARENT_SESSION_UNRESOLVED"
