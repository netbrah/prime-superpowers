...
- **Verified release provenance.** Prime is published as GitHub release assets, not to the npm registry; a registry install of `@earendil-works/pi-coding-agent@0.8.1` returns 404 and that is expected. The release is [PrimeIntellect-ai/prime-agent v0.8.1](https://github.com/PrimeIntellect-ai/prime-agent/releases/tag/v0.8.1). The installable package name is `prime-agent`; `@earendil-works/pi-*` are its internal dependency names. Pin `https://pub-728493de92a943e2a9b2d17b4719f318.r2.dev/releases/v0.8.1/prime-agent-0.8.1.tgz`, whose three internal deps resolve by URL from the same prefix. The release ships `latest.json` with a machine-readable `tarballs[]` array of `{package, file, sha256}`; `--verify-downloads` parses it and asserts agreement with the committed `toolchain/SHA256SUMS` rather than hardcoding. Confirmed in-sandbox: all four published SHA-256 values match bytes recomputed after download, `engines.node` is exactly `>=22.8.0`, `bin` is `{"prime-agent": "dist/bundle/cli.js"}`, the install succeeds, and the installed binary's `--version` prints exactly `0.8.1`.
