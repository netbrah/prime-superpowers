...
The runtime home is composed from exactly four origins and nothing else:

| Path under the runtime home | Origin | Mutability |
|---|---|---|
| `settings.json`, `AGENTS.md`, `extensions/`, `skills/` | byte-identical copy of the committed template | read-only after composition |
| `models.json` | generated by the launcher from the frozen provider profiles | read-only after composition |
| `packages/` | symlink to the shared package cache entry | read-only, cache-owned |
| `auth/`, `sessions/`, `logs/`, `cache/`, `daemon/` | created empty by the launcher | Prime-owned, read-write |

Composition copies regular files only; it never follows or reproduces symlinks out of the template, and it fails closed if the template contains one. Copied files are written `0600`, directories `0700`, and the runtime home is rejected if it is group- or world-accessible. After composition the launcher records the template tree hash and the per-file digests of every copied path in `<run>/agent-home/resources.lock.json` and refuses to start when a recomposition of the same run yields a different digest set.

The run record stores the absolute runtime-home path alongside the target, worktree, branch, and parent session identity. `./prime attach`, `./prime status`, and `./prime stop` reuse that exact recorded path and never recompose it; a missing or digest-divergent runtime home makes the run `orphaned` rather than silently rebuilt. Runtime homes are retained after completion for audit and are removed only by explicit operator cleanup.

### Shared package cache

The pinned Superpowers package is materialized once per kit clone, not once per run. `<kit>/.state/packages/<name>@<ref>/` holds the resolved package tree, and the runtime home's `packages/` entry links to it. Materialization verifies the resolved git commit against the pinned reference and records the tree digest in `<kit>/.state/packages/index.json`. A cache entry whose recomputed digest does not match its recorded digest is rejected and must be rebuilt explicitly.

A run whose required cache entry is already present and digest-valid performs no network access for package resolution, so repeated runs are offline-capable and do not re-clone the pinned repository. When the entry is absent or invalid, the launcher materializes it before Prime is spawned; if that materialization cannot complete, the launcher fails closed with `E_PACKAGE_UNRESOLVED` and never spawns Prime. Prime itself is never relied upon to report an unresolved package, because its resolver intentionally continues with a reduced resource set.
...
Depth one is enforced at dispatch admission and detected on mutation. It is not claimed to be mechanically unbreakable, because Prime 0.8.1 provides no interception point for it.
...
The kit instead relies on three controls that are reachable in the real binary:
...
2. **Controller admission.** The workflow controller is the only sanctioned dispatch path. Before admitting any child it reads the effective depth and the source that produced it, and refuses admission unless the value is one and the source is the kit's global settings. A persisted session override is therefore visible to the controller and blocks further dispatch rather than silently permitting a grandchild.
3. **Ledger detection.** Every admission records the observed depth value and source. A run in which the observed source changes from global settings to persisted chat state is recorded as a governance event, stops the current task, and requires operator acknowledgement before any further admission.

The honest guarantee is that no grandchild can be dispatched through the sanctioned workflow path, and that an out-of-band depth change is detected and halts the run. A user who bypasses the controller entirely inside a live interactive session is outside the workflow contract, exactly as a user who runs raw `git` commands in the worktree is.
...
- Assert the runtime home is composed from the committed template with matching per-file digests, its permissions are owner-only, its `packages/` entry resolves to a digest-valid shared cache entry, a second run performs no package network access, and an absent or corrupt cache entry fails closed with `E_PACKAGE_UNRESOLVED` before Prime is spawned.
- Assert the runtime agent home is active, global `rlmMaxDepth: 1` takes effect even if the operator's normal home says 2, the controller refuses admission when the observed depth source is persisted chat state, the package extension is filtered, and the created worktree is the cwd for root and child sessions. After a worker commit, the coordinator-built `BASE..HEAD` package must contain at least one commit and a non-empty diff, and coordinator/worker ledger paths must resolve identically.
...
- **Runtime topology:** made the committed agent home an immutable template and the per-run composed home the only runtime target, with a frozen path table, copy-versus-create boundary, permissions, symlink rejection, digest lock, and attach/status/stop reuse rules.
...
- **Depth guarantee:** removed the unimplementable interception claim, documented why no extension can intercept the builtin command, and replaced it with template default, controller admission refusal on a non-global depth source, and ledger-recorded detection that halts the run.
