...
Status: draft, round 6 runtime-topology and depth amendment incorporated
...
The kit uses a committed, isolated Prime Agent home rather than depending on project resource discovery from the target directory. The committed `<kit>/agent-home` is an immutable template, never a runtime target. For each run the launcher materializes a runtime home at `<kit>/.state/runs/<run-id>/agent-home`, sets `PRIME_AGENT_CODING_AGENT_DIR` to that runtime home, creates or validates a worktree from the requested target repository, then changes to that worktree before starting Prime Agent. This makes the kit's global settings, extension, skills, depth limit, git context, and artifact root deterministic while keeping the tracked template byte-stable across runs. Target-local `AGENTS.md` and `.prime/agent` resources are visible in the worktree and may add stricter rules.
...
./prime /path/to/target
  ...
  -> composes the per-run runtime home from the committed template
  -> exports the runtime agent home, PI_CACHE_RETENTION=long, and telemetry opt-out
...
### Runtime home topology

The runtime home uses Prime 0.8.1's real agent-directory layout, not an invented one. Every path below is a path Prime actually computes from the agent dir:

| Path under the runtime home | Origin | Integrity class |
|---|---|---|
| `AGENTS.md`, `extensions/`, `skills/` | byte-identical copy of the committed template | launcher-owned, integrity-checked |
| `models.json` | generated by the launcher from the frozen provider profiles | launcher-owned, integrity-checked |
| `git/github.com/obra/superpowers` | symlink to the shared package cache entry | launcher-owned, integrity-checked |
| `resources.lock.json` | generated manifest covering every immutable input above | launcher-owned, integrity-checked |
| `settings.json` | seeded from the template, then rewritten by Prime | launcher-seeded, Prime-mutable, semantically checked |
| `bin/` | symlink to the shared per-clone tool cache | launcher-owned, integrity-checked at composition |
| `auth.json`, `sessions/`, `logs/`, `harness/` | created by the launcher or by Prime at runtime | Prime-owned, read-write |
| `daemon/daemon.sock` | created by Prime under the launcher's explicit socket argument | Prime-owned, read-write |

Prime stores credentials in the file `auth.json`, not a directory, and keeps global RLM refinement state under `harness/`. There is no generic `cache/` root in this topology; Prime's transient extension cache lives in the system temporary directory and is deliberately outside the run.

`settings.json` cannot be digest-locked. Prime's settings manager writes it during normal operation through a temporary file and atomic rename, so any run in which a global setting is persisted would fail a byte-comparison and be falsely declared orphaned. It is therefore checked semantically rather than by digest: the launcher asserts the effective settings still satisfy the required predicates, primarily `rlmMaxDepth: 1`, the pinned package, and `extensions: []`, and records each observed change as a governance event. A rewrite that preserves the predicates is normal operation; one that violates them halts the run.

`bin/` is a real agent-directory path that Prime computes and populates with fetched tools. Left unclassified it would be re-populated per run, which would both break the offline claim and make the network-disabled verification case unreachable. It is therefore linked to a per-clone tool cache alongside the package cache, materialized and digest-verified once, so a second run performs no tool download.

The daemon socket does not follow the agent dir. Prime's default is a process-global path under the system temporary directory, which would let two runs share one daemon and break isolation. The launcher therefore always passes its own internal daemon-socket argument pointing inside the run, on start and on every `attach`, `status`, and `stop` invocation, and records that exact socket path in the run record. On Windows the equivalent is a per-run named pipe rather than a filesystem path, and `prime.cmd` forwards to WSL where the POSIX path applies.

Session location is also escapable. `PRIME_AGENT_SESSION_DIR`, its legacy equivalent, and a `sessionDir` startup setting can redirect sessions out of the runtime home, so all three are treated as protected controls: the launcher strips them from the child environment and refuses to start if a template or target setting attempts to set them.

`resources.lock.json` is a complete manifest over every immutable runtime input, not only copied files. It records per-file digests of the copied template files, the digest of the generated `models.json`, the literal symlink text plus the canonical resolved target of the package link, and the full verified tree digest of that target. Composition copies regular files only; it never follows or reproduces symlinks out of the template and fails closed if the template contains one, while the package link above is a launcher-declared exception recorded in the manifest. The launcher materializes the run into a temporary directory and atomically renames it into place, requires current-user ownership and owner-only permissions on `.state`, the run directory, the cache index, cache entries, and every parent component, and rejects unexpected symlinks in any non-declared component. Note that owner-only permissions do not make these files immutable to their owner; integrity is enforced by manifest comparison, and the residual threat from a same-UID process is accepted and stated rather than claimed away.

The run record stores the absolute runtime-home path, the daemon socket path, and the parent session identity alongside the target, worktree, and branch. `./prime attach`, `./prime status`, and `./prime stop` reuse those exact recorded values and never recompose the home; each recomputes the full manifest before trusting the runtime, and a missing or divergent runtime home makes the run `orphaned` rather than silently rebuilt. Runtime homes are retained after completion for audit and are removed only by explicit operator cleanup.

### Shared package cache

The pinned Superpowers package is materialized once per kit clone, not once per run. `<kit>/.state/packages/<name>@<ref>/` holds the resolved package tree, and the runtime home exposes it at the exact leaf Prime computes for the declared source.

This path is not a matter of preference. For a global git source Prime derives its install path as `<agentDir>/git/<host>/<repository path>`, tests that exact path, and skips cloning only when it already exists. The declared source `git:github.com/obra/superpowers@v6.3.0` therefore resolves at `<runtime-home>/git/github.com/obra/superpowers`, and the launcher creates the parent directories and links that leaf to the verified cache entry. A link placed anywhere else is simply ignored, and Prime clones over the network instead. The launcher derives this mapping from the declared source rather than hardcoding it, preflights that the computed leaf matches the link it created, and rejects a project-local package that would collide with it.

Materialization verifies the resolved git commit against the pinned reference and records the tree digest in `<kit>/.state/packages/index.json`. A cache entry whose recomputed digest does not match its recorded digest is rejected and must be rebuilt explicitly. Cache entries are built in a temporary directory and atomically renamed, and are owned by the current user with owner-only permissions.

A run whose required cache entry is already present and digest-valid performs no network access for package resolution, so repeated runs are offline-capable and do not re-clone the pinned repository. When the entry is absent or invalid, the launcher materializes it before Prime is spawned; if that materialization cannot complete, the launcher fails closed with `E_PACKAGE_UNRESOLVED` and never spawns Prime. Prime itself is never relied upon to report an unresolved package, because its resolver intentionally continues with a reduced resource set. Verification asserts that Prime loads a distinctive Superpowers skill from that leaf with network access disabled.
...
The template `agent-home/settings.json` sets `rlmMaxDepth: 1` and declares `git:github.com/obra/superpowers@v6.3.0` with `extensions: []`. The committed template contains no runtime state; auth, sessions, logs, caches, daemon sockets, and package trees exist only under `<kit>/.state/`, which is ignored by Git in its entirety.
...
Depth one is enforced primarily by Prime itself, which hard-refuses any spawn once a session reaches its maximum depth. The kit adds a launcher-side admission gate and bounded polling to keep that guard correctly configured and to make overrides auditable. The kit's own layer is not claimed to be mechanically unbreakable, because Prime 0.8.1 provides no interception point for the slash command and no atomic check-and-spawn operation.

The kit states the limitation precisely. `/rlm-max-depth` is a builtin slash command consumed inside the interactive submit handler before any submission exists, and extension `input` handlers run only after a submission exists. Prime exposes no extension or hook API for depth, and a builtin command name resolves ahead of any extension-registered command. Therefore no in-process extension can prevent an operator from changing depth during a live interactive session, and persisted chat depth outranks the template's global setting for the remainder of that session.

The observation channel must also be stated precisely, because the controller's default surface cannot see what it needs. The kit controller is invoked from a Prime IPython cell, and that host bridge registers only the RLM run, model-discovery, subagent-listing, and subagent-deletion operations. It exposes no depth-status handler, and the kernel environment exports only a numeric depth value that its own source comments describe as possibly stale, with no indication of which precedence level produced it. A controller reading that variable cannot distinguish a persisted chat override from the kit's global setting, so it must not be the enforcement input.

The reachable channel is Prime's daemon protocol, which returns a typed depth status containing both the value and its source. **That channel must not be handed to the controller.** The same socket and the same dispatcher also accept `set_rlm_max_depth {activeSessionId, maxDepth, global}`, gated only by the session id, and its `global` form persists the change to `settings.json`. The model-invoked controller currently has no depth-write primitive of any kind: the RLM host bridge registers only run, model-discovery, subagent-listing, and subagent-deletion; extensions expose nothing; and the kernel environment is read-only. Giving the controller the socket and the active session id in order to *read* depth would therefore manufacture the exact privilege escalation the guarantee exists to prevent, converting a read requirement into a write capability.

The daemon client is consequently owned by the launcher, on the launcher's side of the trust boundary. The launcher holds the socket path and the session identity, never exports either into the kernel environment or any child process reachable by a model, and performs every depth observation itself. The controller receives only the launcher's decision.

On that basis the kit relies on four controls, in order of strength:

1. **Prime's own spawn guard.** Prime hard-refuses a spawn whenever the session's depth has reached its maximum, throwing before any child is created. This is real, in-process, and not bypassable by the coordinator, and it is the primary enforcement. The kit's controls exist to keep that guard correctly configured and to make violations auditable, not to substitute for it.
2. **Template default.** The runtime home's global settings set `rlmMaxDepth: 1`, so the default posture is depth one even when the operator's normal home says otherwise.
3. **Launcher admission gate.** The workflow controller is the only sanctioned dispatch path, and the launcher answers its admission requests. Before each admission the launcher reads the effective depth and source over its own daemon client and refuses unless the value is one and the source is an accepted origin. Accepted origins are `global` and `inherited`; `chat` and `env` are refused. `inherited` must be accepted because a child session reports its configured depth with that source rather than `global`, so a global-only predicate would be unsatisfiable in every non-root session.
4. **Bounded polling and reconciliation.** Prime offers no atomic check-and-spawn operation, so an observation is a point-in-time read rather than a lock. The launcher polls on the reconciliation cadence, records every observation in the ledger, and treats a transition to `chat` or `env` as a governance event that halts further admission and requires operator acknowledgement. Any child already admitted is reconciled and, where the transition invalidates its dispatch conditions, cancelled through the existing deadline and cleanup path.

The honest guarantee is therefore stronger than earlier rounds claimed in one respect and narrower in another. Stronger: a grandchild cannot be spawned at all while depth is one, because Prime itself refuses it in-process; the kit does not rely on its own admission logic for that. Narrower: an out-of-band depth change made between two observations is detected at the next poll or admission rather than at the instant it happens, and the kit deliberately does not give any model-reachable process the ability to make such a change. A user who bypasses the controller inside a live interactive session is outside the workflow contract, exactly as a user who runs raw `git` commands in the worktree is. The kit does not claim to defend against a hostile operator; it defends against a confused coordinator and an accidental override, and it makes both auditable.
...
Environment files are data, not shell scripts. A parser accepts comments, quoted/unquoted scalar values, and escapes but never executes command substitution or expansion. Precedence from lowest to highest is kit `.env`, target `.env`, kit `.env.local`, target `.env.local`, then the existing process environment. Target files may set public `PRIME_BASE_URL`, model IDs, timeouts, and proxy compatibility values, but cannot set protected controls including `PRIME_AGENT_CODING_AGENT_DIR`, `PRIME_AGENT_SESSION_DIR` and its legacy equivalent, the `sessionDir` startup setting, the daemon socket path, executable path/version, `PI_CACHE_RETENTION`, package/skill/extension settings, model/provider/thinking/cwd CLI overrides, or lock location.
...
The launcher's own invariant options are internal and are placed ahead of user input; the per-run daemon socket is one of them, so the user-facing surface never selects it. The safe surface is limited to positional prompts/file references after the internal options, `-p`/`--print`, `--mode text|json|rpc`, verbosity, and color. It rejects unknown flags; daemon/ACP mode; daemon socket; session-dir/offline/goal controls; split, equal, alias, or repeated provider/model/key/thinking/cwd/system-prompt settings; all extension/skill/tool/theme/template/context selection or `--no-*` disabling; autonomous gates; and resume/continue/fork/no-session. Kit-owned `attach`, `status`, and `stop` are wrapper commands consumed before Prime invocation and operate only on recorded run state. `--unsafe-prime-args` is a separately named escape hatch that prints that workflow guarantees are disabled and requires interactive confirmation; it is unavailable in headless mode.
...
- Assert the runtime home is composed from the committed template with a complete manifest covering copied files, generated `models.json`, and the package link's text plus resolved target tree digest; that ownership and owner-only permissions hold on every component; that the package link sits at Prime's computed `git/<host>/<path>` leaf and Prime loads a distinctive Superpowers skill from it with network access disabled; that a second run performs no package network access; and that an absent or corrupt cache entry fails closed with `E_PACKAGE_UNRESOLVED` before Prime is spawned.
- Assert the launcher passes a per-run daemon socket so two runs cannot share one daemon, that session-directory overrides are stripped and refused, and that attach, status, and stop recompute the full manifest before trusting the runtime.
- Assert `settings.json` is checked semantically rather than by digest: a Prime-initiated rewrite that preserves `rlmMaxDepth: 1`, the pinned package, and `extensions: []` leaves the run healthy and does not orphan it, while a rewrite violating any predicate halts the run.
- Assert `bin/` resolves to the shared tool cache and a second run performs no tool download, so the network-disabled case is reachable.
- **Assert the daemon socket path and active session identity never reach a model-reachable process:** they are absent from the kernel environment, absent from the controller's environment and arguments, and absent from any child process environment. Assert the controller cannot issue `set_rlm_max_depth`, and that a controller attempting a direct daemon connection has no socket path to use.
- Assert the runtime agent home is active, global `rlmMaxDepth: 1` takes effect even if the operator's normal home says 2, the launcher obtains depth value and source over its own daemon client rather than the kernel environment variable, admission is accepted for sources `global` and `inherited`, admission is refused for `chat` and `env`, the gate fails closed on daemon disconnect or session-identity mismatch, the package extension is filtered, and the created worktree is the cwd for root and child sessions.
- Assert Prime's own spawn guard refuses a grandchild in-process while depth is one, independently of the kit's admission gate, by attempting a depth-two spawn with the gate stubbed open.
- After a worker commit, the coordinator-built `BASE..HEAD` package must contain at least one commit and a non-empty diff, and coordinator/worker ledger paths must resolve identically.
...
## Round 6 resolution record

- **Runtime topology:** made the committed agent home an immutable template and the per-run composed home the only runtime target, with a path table drawn from Prime's real layout, a copy-versus-create boundary, ownership and permission rules, symlink handling, a complete integrity manifest, and attach/status/stop reuse rules.
- **Package materialization:** added a per-clone shared package cache so runs do not re-clone the pinned package, made repeat runs offline-capable, and moved `E_PACKAGE_UNRESOLVED` ownership to the launcher preflight because Prime intentionally continues with a reduced resource set.
- **Depth guarantee:** removed the unimplementable interception claim, documented why no extension can intercept the builtin command, moved observation off the stale kernel environment variable onto the daemon depth-status channel, then moved that channel to the launcher's side of the trust boundary after review showed the same socket accepts `set_rlm_max_depth`. Handing it to the model-invoked controller would have created a depth-write primitive that does not otherwise exist. Credited Prime's in-process spawn guard as the primary enforcement, which the previous text under-claimed, and widened the accepted source set to `global` and `inherited` because a child session never reports `global`.
- **Prime-mutable and cached paths:** reclassified `settings.json` as semantically checked rather than digest-locked, because Prime rewrites it in normal operation and a byte check would falsely orphan healthy runs, and added `bin/` as a shared cached path so the offline claim and the network-disabled verification case hold.
- **Real Prime paths:** corrected the runtime layout to Prime 0.8.1's actual agent-directory names, placed the package link at the exact computed git leaf, added a per-run daemon socket because the default socket is process-global, and made session-directory overrides protected controls.
- **Integrity coverage:** extended the manifest beyond copied files to the generated model profile and the package link's canonical target, added ownership, atomic materialization, and symlink-component rules, and replaced the inaccurate read-only claim with an integrity-checked classification that states its same-UID limitation.
