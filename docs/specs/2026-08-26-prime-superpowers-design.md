...
|---|---|---|
| `settings.json`, `AGENTS.md`, `extensions/`, `skills/` | byte-identical copy of the committed template | launcher-owned, integrity-checked |
...
Depth one is enforced by a fail-closed check at every sanctioned dispatch admission and detected by bounded polling between admissions. It is not claimed to be mechanically unbreakable, because Prime 0.8.1 provides no interception point and no atomic check-and-spawn operation.
...
The reachable channel is Prime's daemon protocol, which returns a typed depth status containing both the value and its source. The launcher therefore passes the per-run daemon socket path and the exact parent session identity to the controller, and the controller performs the daemon handshake and queries that status directly. A disconnect, a session-identity mismatch, a replaced session, or any malformed response is treated as fail-closed and refuses admission rather than falling back to the environment variable.

On that channel the kit relies on three controls:

1. **Template default.** The runtime home's global settings set `rlmMaxDepth: 1`, which governs every new session, including every child session, so the default posture is depth one even when the operator's normal home says otherwise.
2. **Controller admission.** The workflow controller is the only sanctioned dispatch path. Immediately before admitting any child it queries the daemon for the effective depth and its source, and refuses admission unless the value is one and the source is the kit's global settings. A persisted session override is therefore visible to the controller and blocks further dispatch rather than silently permitting a grandchild.
3. **Bounded polling and reconciliation.** Prime offers no atomic check-and-spawn operation, so an observation is a point-in-time read rather than a lock. The controller polls the depth status on the same cadence it already uses for child reconciliation, records every observation in the ledger, and treats an observed transition away from the global source as a governance event that halts further admission and requires operator acknowledgement. Any child already admitted before the transition is reconciled and, if the transition invalidates its dispatch conditions, cancelled through the existing deadline and cleanup path.

The honest guarantee is therefore narrow and worth stating plainly. No grandchild can be dispatched through the sanctioned workflow path, because every sanctioned admission is preceded by a fail-closed source check. An out-of-band depth change made between two observations is detected at the next poll or the next admission, not at the instant it happens, and Prime's own spawn guard will have used the changed live value in that window. A user who bypasses the controller entirely inside a live interactive session is outside the workflow contract, exactly as a user who runs raw `git` commands in the worktree is. The kit does not claim to defend against a hostile operator; it defends against a confused coordinator and an accidental override, and it makes both auditable.
...
- Assert the runtime agent home is active, global `rlmMaxDepth: 1` takes effect even if the operator's normal home says 2, the controller obtains depth value and source over the daemon channel rather than the kernel environment variable, it refuses admission when the observed source is persisted chat state, it fails closed on daemon disconnect or session-identity mismatch, the package extension is filtered, and the created worktree is the cwd for root and child sessions. After a worker commit, the coordinator-built `BASE..HEAD` package must contain at least one commit and a non-empty diff, and coordinator/worker ledger paths must resolve identically.
...
- **Depth guarantee:** removed the unimplementable interception claim, documented why no extension can intercept the builtin command, moved observation off the stale kernel environment variable onto the daemon depth-status channel with fail-closed handshake rules, and narrowed the promise to fail-closed refusal at each sanctioned admission plus bounded-poll detection rather than instantaneous prevention.
